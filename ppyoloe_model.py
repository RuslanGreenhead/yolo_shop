"""Private Paddle CPU worker feeding the existing Ultralytics ByteTrack adapter."""
...
class PPYOLOEModel:
    ...
    def track(self, frame, *, persist, tracker, classes, conf, imgsz, device,
        ...
        if device != "cpu" or imgsz != self.size or not nms:
            raise ValueError("PP-YOLOE requires CPU, its configured input size and NMS.")
...
def load_ppyoloe(root: Path):
    ...
    logger.info("PP-YOLOE+ Small ready: Paddle %s, CPU, %d enabled classes",
                metadata["paddle_version"], len(FOOD_CLASSES))
