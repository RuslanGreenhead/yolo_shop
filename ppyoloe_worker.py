"""Paddle 3.3.1 CPU inference in its own Python environment; no ONNX export."""
...
def load_detector(size):
    ...
    if set(state) != set(weights) or any(list(weights[k].shape) != list(v.shape) for k, v in state.items()):
        raise ValueError("Checkpoint parameters do not exactly match PP-YOLOE+ Small Objects365.")
    model.set_state_dict(weights)
    model.eval()
    if model.yolo_head.num_classes != 365:
        raise ValueError("The detector must have the original 365-class Objects365 head.")
    ...
    def predict(frame, threshold):
        ...
        return select_candidates(boxes.numpy(), scores.numpy(), threshold)
    ...
    metadata = {"type": "ready", "labels": labels, "device": paddle.get_device(),
                ...
                "checkpoint_sha256": CHECKPOINT_SHA256, "checkpoint_tensors": len(weights)}
...
def main():
    ...
    with socket.socket(fileno=args.fd) as connection:
        try:
            predict, metadata = load_detector(args.size)
