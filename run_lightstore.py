...
def check_inference():
    ...
    with TestClient(app) as client:
        ...
        state = response.json()
        if (state["enabled_classes"] != manifest["labels"] or state["inference_device"] != "cpu"
    ...
    report = {"platform": platform.platform(), "python": platform.python_version(),
              ...
              "labels": state["enabled_classes"], "device": "cpu", "input_size": 416,
...
def main():
    ...
    if args.check:
        ...
        print(f"LightStore six-class CPU model: http://127.0.0.1:{args.port}/", flush=True)
